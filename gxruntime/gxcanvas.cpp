...
gxCanvas::gxCanvas(gxGraphics* g, IDirect3DSurface9* s, int f) :
	...
	if (flags & CANVAS_TEX_CUBE) {
		...
		HRESULT hr = main_surf->GetContainer(IID_IDirect3DCubeTexture9, (void**)&cube_tex);
		if (SUCCEEDED(hr) && cube_tex) {
			for (int k = 0; k < 6; ++k) {
				if (k == 2) continue;
				D3DCUBEMAP_FACES face;
				switch (k) {
				case 0:face = D3DCUBEMAP_FACE_NEGATIVE_X; break;
				case 1:face = D3DCUBEMAP_FACE_POSITIVE_Z; break;
				case 2:face = D3DCUBEMAP_FACE_POSITIVE_X; break;
				case 3:face = D3DCUBEMAP_FACE_NEGATIVE_Z; break;
				case 4:face = D3DCUBEMAP_FACE_POSITIVE_Y; break;
				case 5:face = D3DCUBEMAP_FACE_NEGATIVE_Y; break;
				default:return;
				}
				cube_tex->GetCubeMapSurface(face, 0, &cube_surfs[k]);
			}
			cube_tex->Release();
			surf = cube_surfs[1];
		}
...
void gxCanvas::backup()const {
	...
	if (!t_surf) {
		D3DSURFACE_DESC desc;
		if (surf->GetDesc(&desc) < 0) return;
		if (desc.Pool == D3DPOOL_SYSTEMMEM) return;
		if (graphics->dir3dDev->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &t_surf, 0)) {
			t_surf = 0;
			return;
		}
	}

	if (FAILED(D3DXLoadSurfaceFromSurface(t_surf, NULL, NULL, surf, NULL, NULL, D3DX_DEFAULT, 0))) return;
}

void gxCanvas::restore()const {
	if (!t_surf) return;

	if (FAILED(graphics->dir3dDev->UpdateSurface(t_surf, NULL, surf, NULL))) return;
}
...
// TODO: Seems that AttachedSurface is replaced with a DepthStencilSurface instead in DX9 for Z-Buffers.
// Not sure which function call is correct for setting this.
bool gxCanvas::attachZBuffer() {
	if (z_surf) return true;
	if (FAILED(graphics->dir3dDev->CreateDepthStencilSurface(getWidth(), getHeight(), graphics->zbuffFmt, D3DMULTISAMPLE_NONE, 0, TRUE, &z_surf, 0))) return false;
	return true;
}
...
void gxCanvas::cls() {
	graphics->dir3dDev->ColorFill(surf, &viewport, clsColor_surf);
	damage(viewport);
}
...
void gxCanvas::plot(int x, int y) {
	...
	Rect dest(x, y, 1, 1);
	graphics->dir3dDev->ColorFill(surf, &dest, color_surf);
	damage(dest);
}
...
void gxCanvas::rect(int x, int y, int w, int h, bool solid) {
	...
	Rect r1(x, y, w, 1); if (clip(&r1)) {
		graphics->dir3dDev->ColorFill(surf, &r1, color_surf);
	}
	Rect r2(x, y, 1, h); if (clip(&r2)) {
		graphics->dir3dDev->ColorFill(surf, &r2, color_surf);
	}
	Rect r3(x + w - 1, y, 1, h); if (clip(&r3)) {
		graphics->dir3dDev->ColorFill(surf, &r3, color_surf);
	}
	Rect r4(x, y + h - 1, w, 1); if (clip(&r4)) {
		graphics->dir3dDev->ColorFill(surf, &r4, color_surf);
	}
	damage(dest);
}
...
void gxCanvas::oval(int x1, int y1, int w, int h, bool solid) {
	...
	if (solid) {
		...
		for (int t = dest.top; t < dest.bottom; ++y, ++t) {
			...
			graphics->dir3dDev->ColorFill(surf, &dr, color_surf);
		}
		damage(dest);
		return;
	}
	...
	for (; t <= hh; ++y, ++t) {
		...
		if (clip(&r1)) graphics->dir3dDev->ColorFill(surf, &r1, color_surf);
		Rect r2(p_xb, t, xb - p_xb, 1); if (r2.left >= r2.right) r2.left = r2.right - 1;
		if (clip(&r2)) graphics->dir3dDev->ColorFill(surf, &r2, color_surf);
		p_xa = xa; p_xb = xb;
	}
...
void gxCanvas::blit(int x, int y, gxCanvas* src, int src_x, int src_y, int src_w, int src_h, bool solid) {
	...
	if (solid) {
		graphics->dir3dDev->StretchRect(src->surf, &src_r, surf, &dest_r, D3DTEXF_NONE);
	}
	else {
		D3DCOLOR colorkey = src->mask_surf;
		if (FAILED(D3DXLoadSurfaceFromSurface(surf, NULL, &dest_r, src->surf, NULL, &src_r, D3DX_DEFAULT, colorkey))) return;
	}
	damage(dest_r);
}
...
void gxCanvas::blitstretch(int x, int y, int w, int h, gxCanvas* src, int src_x, int src_y, int src_w, int src_h, bool solid) {
	...
	graphics->dir3dDev->StretchRect(src->surf, &src_r, surf, &dest_r, D3DTEXF_LINEAR);
	damage(dest_r);
}
...
bool gxCanvas::lock()const {
	if (!locked_cnt++) {
		D3DLOCKED_RECT desc_lr;
		if (FAILED(surf->LockRect(&desc_lr, 0, D3DLOCK_DISCARD))) {
			--locked_cnt;
			return false;
		}
		locked_pitch = desc_lr.Pitch;
		locked_surf = (unsigned char*)desc_lr.pBits;
